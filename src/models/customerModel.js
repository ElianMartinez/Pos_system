...
module.exports = class CustomerModel {
  ...
  async getClientes() {
    const sql = `select * from customer where state = 1 and id_customer > 1 limit 3000`;
    try {
      let result = await db.query(sql);
      return result;
    } catch (err) {
  ...
  async CreateCustomer(name, nick, debit, phone, cedula) {
    try {
      ...
      if (valor == false) {
        let resul = await db.query(
          `call CreateClientes('${name}','${nick}','${cedula}','${phone}',${parseFloat(
            debit
          )})`
        );
        return true;
  ...
  async UpdateDebit(id, debit) {
    var sql = `call updateDebit(${debit},${id});`;
    try {
      let result = await db.query(sql);
