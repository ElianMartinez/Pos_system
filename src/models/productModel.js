  ...
  async EditPrese(data) {
    ...
    try {
      ...
      if (ve.length > 0) {
        ...
        const respe = await bd.query(query);
        if (respe.affectedRows > 0) {
          return "true";
          //insertar
          logController.AddLog(
            data.id_employe,
            `Edito una Presentacion de Producto (${data.id_presen}) (${data.format})`
          );
        } else {
          return "error";
        }
    ...
    try {
      ...
      if (ve.length > 0) {
        ...
        if (re.length > 0) {
          ...
          const a = await bd.query(query);
          //insertar
          logController.AddLog(
            data.id_employe,
            `Regristró una nueva Presentacion de Producto (${data.id_producto}) (${data.format})`
          );
          return "true";
        }
      }
  ...
  async BuscarProducts(data, cont) {
    try {
      ...
      if (cont == 1) {
        ...
        if (data == null) {
          sql =
            "Select p.image, p2.name , p.format,p.num_hierarchy, p.no_unit, p.cantidad, p.price_shopping, p.price_sales_detail, p.price_sales_mayor, p.limit_unit, p.bar_code, p.id_product from presentation p, product p2 where p.id_product = p2.id_product and p.state = 1";
